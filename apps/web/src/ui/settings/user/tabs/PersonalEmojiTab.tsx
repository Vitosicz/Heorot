...
import { PERSONAL_EMOJI_PACK_EVENT_TYPE, type EmojiPackContent } from "../../../emoji/EmojiPackTypes";

interface PersonalEmojiTabProps {
    client: MatrixClient;
...
export function PersonalEmojiTab({ client, onOpenUploadDialog, onToast }: PersonalEmojiTabProps): React.ReactElement {
    const canEdit = !client.isGuest();
    ...
    const loadPack = useCallback(async (): Promise<void> => {
        ...
        try {
            resetEmojiPackCache();
            const loaded = await getPersonalPack(client);
            setPack(clonePack(loaded));
        } catch (loadError) {
            setError(loadError instanceof Error ? loadError.message : "Failed to load personal emoji pack.");
        } finally {
            setLoading(false);
        }
    }, [client]);
    ...
    useEffect(() => {
        const onAccountData = (event: unknown): void => {
            const matrixEvent = event as { getType?: () => string };
            if (matrixEvent.getType?.() !== PERSONAL_EMOJI_PACK_EVENT_TYPE) {
                return;
            }
        ...
        client.on(ClientEvent.AccountData, onAccountData as any);
        return () => {
            client.removeListener(ClientEvent.AccountData, onAccountData as any);
        };
    }, [client, loadPack]);
    ...
    const removeEmoji = async (shortcode: string): Promise<void> => {
        ...
        const confirmed = window.confirm(`Remove ${shortcode} from your personal emoji pack?`);
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            await savePersonalPack(client, nextPack);
            setPack(nextPack);
    ...
    const renameEmoji = async (oldShortcode: string): Promise<void> => {
        ...
        if (pack.emojis[normalized]) {
            setError(`${normalized} already exists in your personal pack.`);
            return;
        }
    ...
    return (
        <div className="settings-tab">
            ...
            <p className="settings-tab-description">
                Manage personal emoji stored as <code>{PERSONAL_EMOJI_PACK_EVENT_TYPE}</code> account data.
            </p>
            ...
            <div className="settings-actions-row">
                ...
                <button
                    ...
                    disabled={!canEdit}
                    title={!canEdit ? "Guests cannot edit personal emoji pack." : "Upload emoji"}
            ...
            {!canEdit ? <p className="settings-inline-note">Read-only: guest sessions cannot edit personal emoji.</p> : null}
